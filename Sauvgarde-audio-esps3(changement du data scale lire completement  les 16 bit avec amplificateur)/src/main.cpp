...
void i2s_adc_data_scale(uint8_t* d_buff, uint8_t* s_buff, uint32_t len) {
  ...
  for (int i = 0; i < len; i += 2) {
    ...
    // Gain x8 pour amplifier le signal (ajuste selon ton micro)
    int32_t amplified = (int32_t)sample * 4;

    // Clamp pour éviter l'écrêtage (saturation propre)
...
void i2s_adc(void* arg) {
  ...
  // Diagnostic : trouve le sample max dans le buffer
int16_t peak = 0;
for (int k = 0; k < bytes_read; k += 2) {
    int16_t s = (int16_t)((i2s_read_buff[k+1] << 8) | i2s_read_buff[k]);
    if (s < 0) s = -s;  // valeur absolue
    if (s > peak) peak = s;
}
Serial.printf("[PEAK] %d / 32767\n", peak);
