...
static const char* getFsmName() {
    ...
    if (s == STATE_ACTION)  return "ACTION";
    if (s == STATE_STANDBY) return "STANDBY";
    if (s == STATE_OFF)     return "OFF";
    return "UNKNOWN";
}
...
static int getLteRssi() {
    ...
    if (rssi == 99) return -999;            // pas de signal
...
static int countAudioFiles() {
    if (!SD_MMC.cardSize()) return 0;
    File dir = SD_MMC.open(AUDIO_DIR_SD);
...
static void handleApiState() {
    ...
    if ((millis() - lastActivityMs) > CAPTIVE_TIMEOUT_MS) shutdownInS = 0;
