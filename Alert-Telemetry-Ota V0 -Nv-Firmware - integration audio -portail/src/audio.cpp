...
static void recordTask(void *arg)
{
    ...
    if (networkTime.length() >= 19)
    {
        ...
        {
            // Format ISO : "2026-06-01T22:15:27.000Z"
            // On extrait YYYY-MM-DD (positions 0-9) et HHMMSS (positions 11,12, 14,15, 17,18)
            snprintf(timeBuf, sizeof(timeBuf),
                     "%c%c%c%c-%c%c-%c%c_%c%c-%c%c-%c%c",
                     networkTime[0], networkTime[1], networkTime[2], networkTime[3], // YYYY
                     networkTime[5], networkTime[6],                                 // MM
                     networkTime[8], networkTime[9],                                 // DD
                     networkTime[11], networkTime[12],                               // HH
                     networkTime[14], networkTime[15],                               // MM
                     networkTime[17], networkTime[18]);                              // SS
        }
        else
        {
    ...
    if (timeBuf[0] != 0)
    {
        // Timestamp réseau disponible
        snprintf(lastFilePath, sizeof(lastFilePath),
                 "%s/alert_%s.wav", AUDIO_DIR_SD, timeBuf);
    }
    else
    {
        // Fallback : pas de date valide, on utilise l'uptime en secondes
        uint32_t uptimeSec = millis() / 1000;
        snprintf(lastFilePath, sizeof(lastFilePath),
                 "%s/alert_uptime_%u.wav", AUDIO_DIR_SD, uptimeSec);
        Serial.println("[AUDIO] WARNING: no valid network time, using uptime fallback");
    }

    // On garde le compteur séquentiel pour les stats (pas pour le nom)
    lastSeq++;
    saveSeq(lastSeq);
