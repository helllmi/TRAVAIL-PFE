...
void mqttLte_loop() {
    ...
    if (line.startsWith("+CMQTTRXSTART")) {
        ...
        while (millis() - start < 6000) {
            if (!ss.available()) { delay(2); continue; }
            String sub = ss.readStringUntil('\n');
            sub.trim();
            if (sub.length() == 0) continue;

            if      (sub.startsWith("+CMQTTRXTOPIC"))   { inTopic = true;  inPayload = false; continue; }
            else if (sub.startsWith("+CMQTTRXPAYLOAD")) { inTopic = false; inPayload = true;  continue; }
            else if (sub.startsWith("+CMQTTRXEND"))     { break; }
            else {
                if (inTopic)   topic   += sub;
                if (inPayload) payload += sub;
            }
        }
