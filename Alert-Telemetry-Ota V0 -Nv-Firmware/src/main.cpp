...
void setup()
{
    ...
    // Wi-Fi (transport MQTT pour l'instant)
...
void loop()
{
    ...
    // ── 7. Envoi TÉLÉMÉTRIE MQTT (toutes les 60s) ──────────────────────────
    ...
    // ── 8. Heartbeat STATE (toutes les 60s, retain=true) ───────── ← NOUVEAU
    if (millis() - lastStateMs > TELEMETRY_INTERVAL_MS)
    {                           // ← NOUVEAU
        lastStateMs = millis(); // ← NOUVEAU
        if (mqttTransport_isConnected())
        {                                           // ← NOUVEAU
            String stateJson = buildStateJson();    // ← NOUVEAU
            Serial.println("[STATE] " + stateJson); // ← NOUVEAU
            mqttTransport_publishState(stateJson);  // ← NOUVEAU
        } // ← NOUVEAU
    }
